...
<body>
 ...
 <h3>
 	...
 	<ul class="side-sub-menu <?php if(!in_array((ACTION_NAME), explode(',',"index"))): ?>subnav-on<?php endif; ?>">
        <li <?php if((ACTION_NAME) == "index"): ?>class="current"<?php endif; ?>><a class="item" href="<?php echo U('banner/index');?>">全部广告</a></li>
 	</ul>
...
<script>
    ...
    <script>
        function setState(obj, id, state, field, onText, offText){
            $.ajax({
                url: "<?php echo U('banner/setState');?>",
        ...
        function delBanner(obj, id){
            if(confirm('确定要删除吗？')){
                $.ajax({
                    url: "<?php echo U('banner/delBanner');?>",
                    data: {id: id},
                    dataType: 'json',
                    type: 'post',
                    success: function(data){
    ...
    <div class="cf">
        <div class="fl">
            <a class="btn" href="<?php echo U("banner/editBanner");?>">添 加</a>
        </div>
    </div>
    ...
    <div class="data-table">
        <table class="">
            ...
            <tbody>
            <?php if(is_array($allbanners)): $i = 0; $__LIST__ = $allbanners;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <td><input class="ids" type="checkbox" name="ids[]" value="<?php echo ($vo["id"]); ?>" /></td>
                    <td><?php echo ($vo["id"]); ?> </td>
                    <td><?php echo ($vo["title"]); ?></td>
                    <td><img width="50" height="50" src="<?php echo ($vo["image"]); ?>"/></td>
                    <td><?php echo ($vo["link"]); ?></td>
                    <td><?php if(($vo["state"]) == "1"): ?><a href="javascript:void(0);" onclick="setState(this, <?php echo ($vo["id"]); ?>, 0, 'state', '发布', '下线')">发布</a><?php else: ?><a href="javascript:void(0);" onclick="setState(this, <?php echo ($vo["id"]); ?>, 1, 'state', '发布', '下线')">下线</a><?php endif; ?></td>
                    <td><?php if(($vo["featured"]) == "1"): ?><a href="javascript:void(0);" onclick="setState(this, <?php echo ($vo["id"]); ?>, 0, 'featured', '是', '否')">是</a><?php else: ?><a href="javascript:void(0);" onclick="setState(this, <?php echo ($vo["id"]); ?>, 1, 'featured', '是', '否')">否</a><?php endif; ?></td>
                    <td><?php echo (date('Y-m-d H:i:s', strtotime($vo["created_time"]))); ?></td>
                    <td><?php echo ($vo["hits"]); ?></td>
                    <td><?php echo ($vo["order"]); ?></td>
                    <td><a href="<?php echo U('Banner/editBanner?id='.$vo['id']);?>">编辑</a>
                        <a href="javascript:void(0);" onclick="delBanner(this, <?php echo ($vo["id"]); ?>)">删除</a>
                    </td>
